...
产品 Agent/Node 均通过 `ContextPolicyFramework`。SimilaritySelectionNode 也先执行 `build("similarity", payload)`，再把该白名单文本封装成最终 semantic-selection Prompt；其 decision 保存 policy metadata，产品测试断言 Memory/RAG/Population sentinel 不会进入文本。
...
Redis key 为 `prievo:run:{run_id}:agent:{scope}:recent`，scope 只能是 `planner`、`generation`、`research`、`repair`。默认 generation 还镜像旧 key `prievo:agent:{run_id}:working_memory`，仅用于数据迁移；planner/research/repair 从不写旧 key。
...
四个 scope 都已进入 Engine 注入的同一 Redis/MySQL 分层路径：

- Planner：每个 `GenerationPlan` 写 `PLANNING_EXPERIENCE`，后续 plan context 可读取 PreviousPlanFeedback 与 planner-scope recent memory。
...
`scripts/ingest_papers.py` 递归扫描 `data/papers/`，将 PDF 解析成统一 paper/section/page/chunk schema，并写入 `pdf_corpus.json`。`pypdf` 是可选依赖；某个坏 PDF 会被明确跳过，不会伪造正文。运行期不联网抓论文，也不会让 Agent 任意浏览文件系统。
...
Tool policy 对 caller、reason、输入 metadata、scope、side effect 和 cost class 做约束；产品 `LiteratureSearchTool` 允许 `HeuristicGenerationAgent` / 兼容 `PriorResearchAgent` 调用 `literature_search`，默认每个 KnowledgeGap/candidate generation scope 上限 3；`CandidateInspectionTool` 允许 `RepairAgent` / `HeuristicGenerationAgent` 检查候选，默认每个 candidate scope 上限 10。两者均为 `READ_ONLY/LOCAL_LOW`。caller、reason、scope、started/finished timestamp、duration、completed/failed/denied 状态会写入 durable ToolCall/Event，工具返回的真实 `tool_call_ref` 继续进入 Evidence 或 Repair Context。
...
SimilaritySelectionNode、RepairAgent、FinalSelectionNode 和 Core selection 不调用 RAG；Repair 只调用本地只读的 CandidateInspectionTool。

## 7. 固定 RAG 评测结果与限制

运行 `python -m prievo_agent.rag_eval ...` 会生成 `reports/rag_eval.json`：默认 curated corpus 包含 3 篇论文、6 个 chunk，5 个 gold query case，`K=3`。该报告评测 `data/literature/corpus.json`，不包含运行时合并的 `pdf_corpus.json`。
