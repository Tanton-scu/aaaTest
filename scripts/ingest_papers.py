...
def _extract_sections(paper_id, pages, chunk_words, overlap_words):
    ...
    sections = []
    for section_ordinal, (section_name, segments) in enumerate(grouped.items()):
        if not segments:
            continue
        units = []
        ...
        while start < len(units):
            ...
            chunks.append(
                {
                    "chunk_id": "{}:{}-{}:{}".format(
                        paper_id, section_ordinal, section_slug, chunk_index
                    ),
...
def main(argv=None):
    ...
    if not list(args.paper_root.rglob("*.pdf")):
