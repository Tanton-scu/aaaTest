...
from prievo_agent.devtools.harness.checkpoint import CheckpointRecoveryHarness
from prievo_agent.runtime.lifecycle import RunLifecycleService
from prievo_agent.devtools.harness.queue import QueueReliabilityHarness
...
class AgentEngineeringHarness:
    ...
    def _mainline(self, root, check):
        ...
        try:
            ...
            completed = PriEvOEngine(
                store,
                DatasetRegistry(self.project_root / "assets" / "datasets"),
    ...
    def _research_resume(self, root, check, *, empty):
        ...
        try:
            ...
            store.add_task(task)
            store.add_run(run)
            skills = SkillRegistry(self.project_root / "assets" / "skills")
    ...
    def _final_tie(self, root, check):
        ...
        try:
            ...
            selected, decision_ref, decision = DurableFinalSelectionWorkflow(
                store, SkillRegistry(self.project_root / "assets" / "skills"), model
    ...
    def _repair_limit(self, root, check):
        ...
        try:
            ...
            workflow = DurableRepairWorkflow(
                store,
                SkillRegistry(self.project_root / "assets" / "skills"),
                model,
