"""Durable EvolutionPlannerNode workflow."""
...
class DurablePlanningWorkflow:
    ...
    def __init__(self, store, model, working_memory=None):
        self.store = store
        self.agent = EvolutionPlannerNode(model)
    ...
    def plan(
        ...
        if created:
            self.store.append_event(
                run.id,
                "GENERATION_PLANNED",
                "EvolutionPlannerNode 已产生结构化 GenerationPlan",
                plan_id=plan.plan_id,
    ...
    def _record_trace(self, plan, context_ref, plan_ref):
        ...
        self.store.record_trace(
            TraceRecord(
                ...
                actor=EvolutionPlannerNode.name,
                status="COMPLETED",
                plan_id=plan.plan_id,
    ...
    def _record_memory(self, run, task, plan, context_ref, plan_ref):
        ...
        self.store.add_agent_memory(
            AgentMemory(
                ...
                agent_name=EvolutionPlannerNode.name,
