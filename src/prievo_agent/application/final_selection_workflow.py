...
class FinalSelectionTaskHandler:
    name = FinalSelectionAgent.name
    capability = FinalSelectionAgent.capability
...
class DurableFinalSelectionWorkflow:
    ...
    def __init__(self, store, skill_registry, model, *, faithful_mode=False):
        ...
        agent = FinalSelectionAgent(model, selector=selector)
        handler = FinalSelectionTaskHandler(store, agent, skill)
        self.agent = agent
        self.skill = skill
        ...
        self.dispatcher = AgentTaskDispatcher(
            ...
            FinalSelectionArtifactWriter(store),
        )
    ...
    def select(self, run_id, candidates, candidate_budget):
        ...
        self.store.append_event(
            ...
            "最终合格候选出现 exact objective tie，路由 FinalSelectionAgent",
            artifact_id=tie.id,
            tied_candidate_ids=payload["tied_candidate_ids"],
            best_objective=resolution.best_objective,
        )
        self.coordinator.reconcile(run_id)
        tasks = [
            item for item in self.store.agent_tasks_for_run(run_id)
            if item.task_type == "FINAL_SELECTION"
            and tie.id in item.input_artifact_refs
        ]
        if len(tasks) != 1:
            raise RuntimeError("无法为 FINAL_TIE 唯一定位 FinalSelectionTask")
        dispatched = self.dispatcher.dispatch_with_retries(tasks[0].id)
        decision_refs = [
            ref for ref in dispatched.artifact_refs
            if any(
                item.id == ref and item.kind == "FINAL_SELECTION_DECISION"
                for item in self.store.artifacts_for_run(run_id)
            )
        ]
        if len(decision_refs) != 1:
            raise RuntimeError("FinalSelectionTask 未产生唯一 decision")
        decision_payload = json.loads(self.store.artifact_content(decision_refs[0]))
        return decision_payload["selected_candidate_id"], decision_refs[0], decision_payload
