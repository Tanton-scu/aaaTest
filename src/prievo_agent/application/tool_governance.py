    ...
    def execute_with_audit(
        ...
        if caller not in policy.allowed_callers:
            ...
            if prior_calls >= policy.max_calls_per_run:
                denied_reason = "tool per-run 调用上限已用尽"
        if denied_reason:
            finished_at = utc_now()
...
class CandidateInspectionTool:
    policy = ToolPolicy(
        "candidate_inspection", frozenset({"RepairAgent"}), 12,
        "READ_ONLY", "LOCAL_LOW",
    )
    ...
    def inspect(self, run_id, candidate_id, reason, caller="RepairAgent"):
        inspection, call_id = self.gateway.execute_with_audit(
            ...
            input_metadata={"candidate_id": candidate_id},
        )
