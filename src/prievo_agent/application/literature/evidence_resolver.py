"""由 KnowledgeGap 触发的纯领域 LiteratureEvidenceResolver。
...
class ResearchSkill:
    ...
    def __post_init__(self):
        if self.name not in {"prior_explanation", "literature_evidence_review"}:
            raise ValueError(
                "LiteratureEvidenceResolver 只允许 prior_explanation 或 "
...
class LiteratureEvidenceModel(Protocol):
...
@dataclass(frozen=True)
class LiteratureEvidenceResolution:
    run_id: str
...
class LiteratureEvidenceResolver:
    name = "LiteratureEvidenceResolver"
    ...
    def research(
        ...
        return LiteratureEvidenceResolution(
            run_id=str(run_id),
...
def _validate_research_skill(skill, expected_name):
    if skill.name != expected_name:
        raise ValueError(
            "LiteratureEvidenceResolver {} 阶段必须使用 {} Skill，收到 {}".format(
...
def _query_prompt(skill, context, context_ref, original_prior_ref):
    return (
        "You are LiteratureEvidenceResolver. Form one bounded literature query for the "
...
def _explanation_prompt(
    ...
    return (
        "You are LiteratureEvidenceResolver. Explain the immutable Original Prior only "
