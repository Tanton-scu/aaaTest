...
class DurableGenerationWorkflow:
    ...
    def __init__(
        ...
        self.store = store
        self.prior_research_workflow = prior_research_workflow
    ...
    def generate(
        ...
        if self.prior_research_workflow is None:
            # 兼容纯领域/旧 composition；产品 Engine 会注入 durable Research workflow。
            return output, request.id, output_ref

        resolution = self.prior_research_workflow.resolve(run.id, output_ref)
        ...
        if isinstance(resumed, KnowledgeGap):
            raise RuntimeError(
                "bounded PriorResearch 后 GenerationAgent 仍返回 KnowledgeGap；"
