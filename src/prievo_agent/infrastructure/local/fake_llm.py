...
from prievo_agent.knowledge.prior.models import LANDSCAPE_METRICS
...
class FakeLLM:
    ...
    def __init__(self) -> None:
        ...
        self.planner_calls: List[Dict[str, object]] = []
    ...
    def export_state(self) -> dict:
        return {
            ...
            "planner_calls": list(self.planner_calls),
    ...
    def import_state(self, state: dict) -> None:
        ...
        self.planner_calls = [
            dict(item) for item in state.get("planner_calls", [])
        ]
    ...
    def plan_generation(self, prompt: str) -> dict:
        strategy_data = _prompt_json(prompt, "Allowed PriEvO strategies") or []
        scheduled = "i1"
        for candidate in ("i1", "e1", "e2", "m1", "m2"):
            if "scheduled strategy {}".format(candidate) in prompt:
                scheduled = candidate
                break
        policy = "null" if scheduled == "i1" else "roulette"
        required = {"i1": 0, "e1": 2, "e2": 2, "m1": 1, "m2": 1}[scheduled]
        self.planner_calls.append({
            "prompt": prompt,
            "scheduled_strategy": scheduled,
            "allowed_strategies": strategy_data,
        })
        return {
            "generation_strategy": scheduled,
            "parent_selection_policy": policy,
            "decision_reason": (
                "Deterministic FakeLLM keeps the Runtime scheduled PriEvO operator "
                "and uses the default parent policy."
            ),
            "required_parent_count": required,
        }
