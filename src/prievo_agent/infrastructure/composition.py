...
from prievo_agent.infrastructure.llm import configured_llm_backend
...
class RuntimeComposition:
    ...
    def __init__(self, runtime_root: Path, project_root=None) -> None:
        ...
        self.dataset_registry = DatasetRegistry(self.project_root / "assets" / "datasets")
        self.runtime_mode = _runtime_mode_from_environment()
        self.database_url = os.getenv("DATABASE_URL", DEFAULT_DATABASE_URL).strip()
        if self.runtime_mode == "full" and not self.database_url.startswith("mysql+pymysql://"):
            raise RuntimeError(
                "DATABASE_URL 必须是 mysql+pymysql://...；本项目只保留 MySQL 完整模式"
            )
        self.evaluation_execution_mode = (
            "inline" if self.runtime_mode == "demo" else "external"
        )
        ...
        redis_default = "" if self.runtime_mode == "demo" else DEFAULT_REDIS_URL
        redis_url = os.getenv("REDIS_URL", redis_default).strip()
    ...
    def health_snapshot(self):
        ...
        if self.runtime_mode == "demo":
            worker = {
                "status": "NOT_REQUIRED",
                "verification": "Demo 模式在 API 进程内执行候选评估",
        ...
        return {
            ...
            "database": "sqlite" if self.runtime_mode == "demo" else "mysql",
    ...
    def open_store(self):
        with self._store_init_lock:
            ensure_schema = not self._schema_ready
            if self.runtime_mode == "demo":
                from prievo_agent.infrastructure.local.sqlite_store import (
                    SQLiteRuntimeStore,
                )

                store = SQLiteRuntimeStore(
                    self.runtime_root / "state.sqlite3",
            ...
            try:
                if self.runtime_mode == "full" and not self._datasets_synced:
    ...
    def execute(self, run_id: str) -> None:
        ...
        try:
            PriEvOEngine(
                ...
                self.project_root / "assets" / "prior",
...
def _runtime_mode_from_environment():
    ...
    if mode not in {"demo", "full"}:
        raise ValueError("RUNTIME_MODE 必须是以下值之一：demo, full")
    return mode
