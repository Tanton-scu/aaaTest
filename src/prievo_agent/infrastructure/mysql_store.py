...
from prievo_agent.domain.models import (
    ...
    OptimizationTask, Run, RunStatus,
...
class MySQLRuntimeStore:
    ...
    def add_candidate(self, candidate):
        with self.connection.cursor() as cursor:
            cursor.execute("""INSERT INTO candidates VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
    ...
    def _candidate(self, row):
        return Candidate(row["id"],row["run_id"],row["code"],row["description"],
                         self._json(row["operators_json"]),self._json(row["lineage_json"]),
