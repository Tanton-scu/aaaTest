"""KnowledgeGap -> durable LiteratureEvidenceTask -> evidence/explanation 的产品工作流。"""
...
class LiteratureEvidenceModelAdapter:
    """把产品/Fake model 收敛为 LiteratureEvidenceResolver 的两个局部能力。"""
...
class LiteratureRetrieverAdapter:
    ...
    def search(self, query):
        backend_query = LiteratureQuery(
            ...
            reason="LiteratureEvidenceResolver KnowledgeGap {}".format(
...
class LiteratureSearchTool:
    """受治理、可追溯 Literature Search Tool。"""

    policy = ToolPolicy(
        "literature_search",
        frozenset({"HeuristicGenerationAgent", "LiteratureEvidenceResolver"}),
        3,
        "READ_ONLY",
        "LOCAL_LOW",
    )
    ...
    def execute(
        self, run_id, caller, tool_name, reason, operation, tool_budget_scope=""
    ):
        if tool_name != self.policy.tool_name:
            raise ValueError("LiteratureEvidence workflow 只允许 literature_search")
        result, call_id = self.gateway.execute_with_audit(
            ...
            input_metadata={
                "tool_name": tool_name,
                "tool_budget_scope": tool_budget_scope or reason,
            },
        )
        return ToolExecution(call_id, tuple(result), "COMPLETED")


class LiteratureEvidenceTaskHandler:
    name = LiteratureEvidenceResolver.name
    capability = AgentCapability.LITERATURE_EVIDENCE
    ...
    def handle(self, task, board):
        if len(task.input_artifact_refs) != 1:
            raise ValueError("LiteratureEvidenceTask 必须且只能引用一个 KNOWLEDGE_GAP")
        gap_artifact = board.artifact_by_ref(task.input_artifact_refs[0])
        if gap_artifact.kind != "KNOWLEDGE_GAP":
            raise ValueError("LiteratureEvidenceTask 输入必须是 KNOWLEDGE_GAP")
...
class PriorResearchArtifactWriter:
    ...
    def __call__(self, task, result):
        ...
        self.store.append_event(
            ...
            "LiteratureEvidenceResolver 已完成有界研究并保留 Original Prior",
            agent_task_id=task.id,
            knowledge_gap_artifact_id=result.knowledge_gap.ref,
...
class PriorResearchResolution:
    ...
    def generation_evidence(self):
        return {
            "run_id": self.run_id,
            "annotation_type": "LITERATURE_EVIDENCE",
...
class DurableLiteratureEvidenceWorkflow:
    """用 Coordinator/Dispatcher 执行且可恢复的唯一 LiteratureEvidenceTask。"""
    ...
    def __init__(
        ...
        agent = LiteratureEvidenceResolver(
            LiteratureEvidenceModelAdapter(model),
            LiteratureRetrieverAdapter(literature_backend),
            LiteratureSearchTool(store),
            max_results=5,
        )
        handler = LiteratureEvidenceTaskHandler(
            store,
            skill_registry,
            agent,
            self.memory,
    ...
    def resolve(self, run_id, knowledge_gap_artifact_id):
        ...
        tasks = [
            ...
            if item.task_type == "LITERATURE_EVIDENCE"
            and item.input_artifact_refs == [knowledge_gap_artifact_id]
        ]
        if len(tasks) != 1:
            raise RuntimeError("KnowledgeGap 未映射到唯一 LiteratureEvidenceTask")
        if any(item.id == tasks[0].id for item in created):
            self.store.append_event(
                run_id,
                "LITERATURE_EVIDENCE_REQUESTED",
                "KnowledgeGap 已派生唯一 LiteratureEvidenceTask",
    ...
    def _remember(self, resolution):
        ...
        if memory.id not in before:
            self.store.append_event(
                ...
                "LiteratureEvidenceResolver 已写入同 Run research scope 历史",
    ...
    def _resolution(self, run_id, gap_ref, task_id, refs):
        ...
        if not required.issubset(by_kind):
            raise RuntimeError("LiteratureEvidenceTask durable outputs 不完整")
        ...
        return PriorResearchResolution(
            ...
            query_prompt_artifact_id=by_kind["LITERATURE_EVIDENCE_QUERY_PROMPT"].id,
