...
from prievo_agent.application.workflows.planning_workflow import DurablePlanningWorkflow
...
from prievo_agent.knowledge.prior.landscape import LandscapeAnalysisService
...
from prievo_agent.knowledge.literature.retrieval.hybrid import build_literature_rag_from_env
from prievo_agent.infrastructure.llm import configured_llm
...
from prievo_agent.evaluation.queue import utc_clock
...
class PriEvOEngine:
    ...
    这个类负责把“面向数据集的 PriEvO 算法流程”和“后端 Agent Runtime”接起来：
    先完成 FLA、相似实例选择和 instance-specific prior，再把候选生成、评价、修复、
    checkpoint、final selection/optimization 交给 PersistentEvolutionRuntime。
    """
    ...
    def __init__(
        ...
        if self.llm is None:
            raise RuntimeError(
                "未配置 LLM；请填写 LLM_API_ENDPOINT、LLM_MODEL 和 LLM_API_KEY/ARK_API_KEY"
            )
    ...
    def prepare(self, run_id, runtime_owner_id=""):
        ...
        prepare 本身也在 Run lease 内执行，避免多个 App 实例对同一个 Run 重复做
        FLA / Similarity / Prior artifact 写入。
        """
        ...
        analysis = LandscapeAnalysisService(recorded_repository=repository).analyze(
            dataset,
        ...
        self.store.append_event(
            run.id,
            "LANDSCAPE_SAMPLED",
            "已完成目标 Dataset 的确定性地貌采样",
            dataset_id=run.dataset_id,
            dataset_digest=dataset.info.digest,
            sample_count=sample_size,
            sample_artifact_id=sample_artifact.id,
            fla_source=analysis.source,
            fla_fallback_reason=analysis.fallback_reason,
        )

        logger.info(
            "run_id=%s 正在检索 Dataset %s 的 instance-specific prior",
            run.id,
            run.dataset_id,
        )
        project_root = self.prior_root.parents[1]
        skill_registry = SkillRegistry(project_root / "assets" / "skills")
        retrieval = PriorRetrievalService(repository)

        # Numeric Top-5 与 semantic decision 分层：数值检索给候选实例，LLM 只做语义精选。
        ...
        compatibility_artifact = self.store.put_artifact(
            ...
            json.dumps(compatibility_payload, ensure_ascii=False, sort_keys=True).encode(
                "utf-8"
            ),
            "application/json",
        )
        ...
        self.store.append_event(
            run.id,
            "LANDSCAPE_ANALYZED",
            "目标 Dataset 的 FLA 与 prior 已准备完成",
            dataset_id=run.dataset_id,
            dataset_digest=dataset.info.digest,
    ...
    def run(self, run_id):
        """获取 Run lease，完成 prepare，然后移交给持久化演化 runtime。"""
        ...
        if not self.store.claim_run_lease(
            ...
            if current.status in {RunStatus.PAUSED, RunStatus.CANCELLED}:
                return current
            raise RuntimeLeaseConflict("Run 已被其他 Runtime owner 持有：{}".format(run_id))
    ...
    def _run_owned(self, run_id, context, runtime_owner_id):
        ...
        # 文献检索不是一个独立“先验研究大脑”。它是 HeuristicGenerationAgent 在遇到
        # KnowledgeGap 时可以通过治理网关调用的 read-only tool workflow。
        literature_evidence_workflow = None
        if not self.research_faithful_mode:
            literature_evidence_workflow = DurableLiteratureEvidenceWorkflow(
                self.store,
                skill_registry,
                self.llm,
                build_literature_rag_from_env(
                    [
                        project_root / "assets" / "literature" / "corpus.json",
                        project_root / "assets" / "literature" / "pdf_corpus.json",
                    ]
                ),
                working_memory=self.agent_working_memory,
            )

        # App 进程只创建 durable EvaluationJob；真正的候选评价由独立 worker 消费。
        # 这里仍构造 DatasetEvaluator，是为了让 job identity 包含与 worker 一致的 metadata。
        ...
        runtime = PersistentEvolutionRuntime(
            ...
            generation_workflow=DurableGenerationWorkflow(
                ...
                skill_registry,
                self.llm,
                literature_evidence_workflow=literature_evidence_workflow,
                working_memory=self.agent_working_memory,
            ),
...
def _metric_semantics(path):
    """读取 reference-compatible metric definitions，给 Similarity Agent 解释指标含义。"""
