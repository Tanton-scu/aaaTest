...
class PersistentEvolutionRuntime:
    ...
    def _generate_durable_batch(
        ...
        for index in range(self.core.population_size):
            ...
            parents = self.core.select_generation_parents(population, operator)
            offspring.append(
                self._generate_durable_candidate(
                    run, task, population, generation, operator, index, parents
                )
            )
        return offspring

    def _generate_durable_candidate(
        self, run, task, population, generation, operator, index, parents
    ):
        from prievo_agent.agents.heuristic_generation import KnowledgeGap
        ...
        # 先保存 CREATED Candidate，再进入 validation/artifact/job 阶段。即使
