...
class AgentEngineeringHarness:
    ...
    def _mainline(self, root, check):
        ...
        try:
            ...
            expected_counts = {
                ...
                "SEMANTIC_SIMILARITY_SELECTION": 1,
            }
    ...
    def _final_tie(self, root, check):
        ...
        try:
            ...
            check.equal("final_task_cardinality", 1, len(tasks))
            ...
            check.subsequence(
                "final_tie_trace",
                ["FINAL_TIE_DETECTED", "AGENT_TASK_CREATED", "AGENT_TASK_CLAIMED", "AGENT_TASK_COMPLETED"],
                events,
            )
    ...
    def _duplicate_event(self, root, check):
        ...
        try:
            ...
            store.add_task(task)
            store.add_run(run)
            payload = json.dumps({"fixture": "top5"}, sort_keys=True).encode("utf-8")
            first = store.put_artifact(run.id, "TOP5_CANDIDATES", payload, "application/json")
            second = store.put_artifact(run.id, "TOP5_CANDIDATES", payload, "application/json")
            for _ in range(2):
                store.append_event(
                    run.id,
                    "SIMILARITY_CANDIDATES_READY",
            ...
            check.equal("duplicate_events_retained_for_audit", 2, sum(
                item.event_type == "SIMILARITY_CANDIDATES_READY"
