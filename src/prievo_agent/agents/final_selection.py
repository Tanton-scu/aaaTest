...
class FinalSelectionSkill:
    ...
    def __post_init__(self):
        if self.name != "final_heuristic_audit":
            raise ValueError("FinalSelectionAgent 必须使用 final_heuristic_audit Skill")
...
def _final_prompt(skill, context, context_ref, candidates):
    ...
    return (
        "You are FinalSelectionAgent. All supplied candidates have the exact "
