...
from .models import SimilarityDecision
from .context_policies import ContextPolicyFramework


class SimilarityAgentError(RuntimeError):
    """SimilarityAgent 无法产生有效决策。"""


class MalformedSimilarityDecisionError(SimilarityAgentError):
...
class SimilaritySelectionNode:
    ...
    def select(
        ...
        if self.model is None or not hasattr(
            ...
            raise SimilarityAgentError("SimilaritySelectionNode 未配置 semantic selection model")
        ...
        try:
            ...
            raise MalformedSimilarityDecisionError(
                "SimilarityAgent 响应不是有效 JSON object：{}".format(exc)
            ) from exc
    ...
    def _validate_inputs(
        ...
        if len(candidates) > 5:
            raise ValueError("SimilarityAgent 只接受 numeric Top-5（最多 5 个候选）")
...
class SimilarityAgent(SimilaritySelectionNode):
    """Backward-compatible Agent facade for legacy tests and stored AgentTasks.

    新架构中的真实调用链应直接使用 ``SimilaritySelectionNode``；这里保留旧名称
    和 capability，只服务于历史导入、测试 fixture 与尚未迁移的持久任务。
    """
