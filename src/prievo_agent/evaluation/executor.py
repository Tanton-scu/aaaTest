        ...
        with tempfile.TemporaryDirectory(prefix="prievo-heuristic-") as directory:
            ...
            if output_path.exists() and output_path.stat().st_size <= 2 * 1024 * 1024:
                ...
                if response.get("status") == "ok" and return_code == 0:
                    return response
                _raise_worker_failure(response, return_code)

            _raise_missing_output(return_code)
...
def _thread_env(name):
    value = os.getenv(name, "1").strip() or "1"
    try:
        parsed = int(value)
    except ValueError as exc:
        raise CandidateInterfaceError("{} 必须是整数".format(name)) from exc
    if parsed < 1 or parsed > 64:
        raise CandidateInterfaceError("{} 必须在 1..64".format(name))
    return str(parsed)
