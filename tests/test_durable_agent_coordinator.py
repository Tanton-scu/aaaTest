...
class DurableAgentCoordinatorTest(unittest.TestCase):
    ...
    def test_reconcile_derives_five_unique_tasks_and_only_emits_created_events(self):
        ...
        self.assertEqual(5, len({task.id for task in all_created}))
        self.assertEqual(5, len({task.idempotency_key for task in all_created}))
    ...
    def test_sweep_is_stable_deduplicates_run_ids_and_is_idempotent(self):
        ...
        artifact_a = self._artifact(run_a.id, "FINAL_TIE")
        artifact_b = self._artifact(run_b.id, "TOP5_CANDIDATES")
        coordinator = DurableAgentCoordinator(self.store)
