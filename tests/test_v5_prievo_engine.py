    ...
    def test_dataset_is_bound_through_prior_evaluation_and_checkpoint(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                planner_memories = list(
                    store.agent_memories_for_run(run.id, "planner", limit=40)
                )
            finally:
                store.close()
        ...
        self.assertEqual(
            {"i1", "e1", "e2", "m1", "m2"},
            {item.generation_strategy for item in plans},
        )
        self.assertTrue(all(item.plan_id for item in plans))
        self.assertTrue(all(item.context_artifact_id for item in plans))
        self.assertTrue(all(item.prompt_artifact_id for item in plans))
        self.assertEqual(
            16,
            sum(item.span_type == "LLM_AGENT_PLAN" for item in traces),
        )
        self.assertEqual(16, len(planner_memories))
        self.assertEqual(
            16,
        ...
        self.assertTrue(all(item.plan_id for item in evolved))
        self.assertTrue(all(item.generation_strategy for item in evolved))
        self.assertTrue(all(item.code_digest for item in evolved))
        self.assertTrue(all(
            item.selected_parent_ids == list(item.lineage.get("parents", []))
            for item in evolved
        ))
