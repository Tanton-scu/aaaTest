...
class DurableFinalSelectionWorkflowTest(unittest.TestCase):
    def test_unique_direct_and_exact_tie_node_paths_are_distinct(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                artifact_kinds = {
                    item.kind for item in store.artifacts_for_run(tie_run.id)
                }
                self.assertIn("FINAL_TIE", artifact_kinds)
                self.assertIn("FINAL_SELECTION_PROMPT", artifact_kinds)
                self.assertIn("FINAL_SELECTION_DECISION", artifact_kinds)
