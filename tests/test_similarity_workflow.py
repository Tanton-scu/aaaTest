...
class DurableSimilarityWorkflowTest(unittest.TestCase):
    def test_top5_task_agent_decision_then_repository_prior(self):
        with tempfile.TemporaryDirectory(prefix="prievo-similarity-flow-") as directory:
            ...
            try:
                ...
                self.assertEqual("COMPLETED", agent_tasks[0].status.value)
                self.assertIn(top5_ref, agent_tasks[0].input_artifact_refs)
                self.assertIn(decision_ref, agent_tasks[0].output_artifact_refs)
